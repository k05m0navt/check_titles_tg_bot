...
11. **User Registration & Group Management**
    ...
    - Admin can add users manually via `/add_user @username` command (admin only)
    - Track group membership (which users are in which groups)
...
12. **Default Title System**
    ...
    - Admin can update default title via `/set_default_title <title>` command
...
- [ ] **Task 2.5: Settings Repository**
  ...
  - **Action**: `set_default_title(title: str)` updates `bot_settings.default_title` setting
...
- [ ] **Task 3.7: User Registration Use Cases**
  - **File**: Create `src/application/use_cases/register_user_use_case.py`
  ...
  - **Action**: Save user to repository
  - **Action**: Return success message indicating registration status
  - **File**: Create `tests/unit/test_register_user_use_case.py`
  - **Action**: Unit tests with mocked dependencies, test existing user (idempotent), test default title assignment, test empty default title
...
- [ ] **Task 3.8: Admin Commands Use Cases**
  ...
  - **Action**: Implement `AddUserUseCase` class with `execute(username: str, admin_user_id: int, admin_username: str, bot_instance)` method
  - **Action**: Validate admin, resolve username to Telegram user ID using `bot_instance.get_chat(username)` or similar API
  - **Action**: If username doesn't exist in Telegram, raise `UserNotFoundError` with friendly message
  - **Action**: If user already exists in database, return success (idempotent)
  - **Action**: Get default title from settings, create user with default title as `full_title`
  - **Action**: Save user to repository
  ...
  - **Action**: Validate admin, update `bot_settings.default_title` using settings repository
  - **Action**: Existing users are NOT updated (they keep their current `full_title`)
  - **Action**: Only new registrations will use the new default title
  - **File**: Create `tests/unit/test_add_user_use_case.py`
  - **Action**: Unit tests with mocked bot instance, test username resolution, test existing user (idempotent)
  - **File**: Create `tests/unit/test_set_default_title_use_case.py`
  - **Action**: Unit tests with mocked dependencies, test admin validation, test settings update
...
- [ ] **Task 4.1: Command Handlers**
  ...
  - **Action**: Implement `/register` handler - call `RegisterUserUseCase`, show success message with welcome text
  - **Action**: Update `/start` handler - show welcome message, optionally mention `/register` command for registration
  - **Action**: Implement `/add_user @username` handler (admin only) - call `AddUserUseCase`, show success or error message
  - **Action**: Implement `/set_default_title <title>` handler (admin only) - call `SetDefaultTitleUseCase`, show confirmation message
...
- [ ] **Task 4.5: Chat Member Handler (Group Events)**
  ...
  - **Action**: Check if bot itself is in `new_chat_members` list (when bot is added to group)
  - **Action**: When bot is added to group: Send welcome message explaining registration process and `/register` command
  - **Action**: Optional: Use `ChatMemberUpdated` handler for more detailed bot membership tracking
  - **Action**: Error handling and logging
  - **File**: Update `src/presentation/main.py`
  - **Action**: Register `MessageHandler` with `filters.StatusUpdate.NEW_CHAT_MEMBERS` filter for bot-added-to-group events
  - **Notes**: Welcome message should explain how users can register via `/register` command
...
- [ ] **Task 4.8: Application Entry Point**
  ...
  - **Action**: Register all handlers: CommandHandler, CallbackQueryHandler, MessageHandler, InlineQueryHandler
  ...
  - **Notes**: Use manual dependency injection (pass dependencies to handlers)
...
- **Given**: Admin runs `/add_user @username` and default title is set in bot settings
- **And**: Username exists in Telegram and is not already in database
- **When**: Admin command is processed
- **Then**: Username is resolved to Telegram user ID via Bot API
...
- **Given**: Admin runs `/add_user @nonexistent_user`
- **When**: Bot API fails to resolve username (user doesn't exist in Telegram)
- **Then**: `UserNotFoundError` is raised with friendly message
- **And**: Admin receives error message: "❌ User @nonexistent_user not found in Telegram. Please check the username."
- **And**: No database changes are made

**AC40: Admin Add User - User Already Exists (Idempotent)**
- **Given**: User already exists in database
- **When**: Admin runs `/add_user @existing_user`
...
- **Then**: `bot_settings.default_title` is updated to "Super Gay Title"
...
**AC44: Bot Added to Group - Welcome Message**
...
**AC45: Bot Added to Group - Multiple Members Added**
